...
// Загрузка контента категории с поддержкой тематических категорий
...
// Загрузка проектов с поддержкой бесконечного скролла
async function loadProjects(contentType = 'all', append = false) {
    ...
    try {
        ...
        // Добавляем бесконечный скролл для результатов поиска
        if (hasMore && !append) {
...
// Добавление бесконечного скролла для результатов поиска
...
async function loadRecommendations() {
    ...
    try {
        ...
        // Единая лента для всех типов контента (каналы, боты, приложения)
        recommendations.forEach(project => {
            const card = createProjectCard(project, false); // false = не показывать причину
...
// Загрузка категорий
// Загрузка категорий с переходом на отдельные страницы
...
// Открытие страницы тематической категории
...
// Загрузка контента тематической категории
...
function createProjectCard(project, showReason = false) {
    ...
    // Поддержка разных полей для количества подписчиков
    ...
    // Показываем подписчиков только для каналов
    const showSubscribers = projectType === 'channel' || project.type === 'channel';

    // Badge для типа проекта (только для ботов и мини-приложений)
    let typeBadge = '';
    if (projectType === 'bot') {
        typeBadge = '<span class="type-badge type-badge-bot">Бот</span>';
    } else if (projectType === 'mini_app') {
        typeBadge = '<span class="type-badge type-badge-app">Приложение</span>';
    }
...
// Обработка поиска с закрытием клавиатуры
...
❓ Что такое TCatalog?
✅ Это умное приложение для поиска каналов, ботов и мини-приложений в Telegram с персонализированными рекомендациями.

❓ Как работает приложение?
✅ Приложение анализирует ваши предпочтения и предлагает релевантный контент через поиск и систему рекомендаций.

🔍 РАБОТА С ПОИСКОМ

❓ Как искать контент?
✅ Используйте строку поиска вверху экрана:
• Введите ключевые слова и нажмите Enter
• Используйте фильтры для уточнения результатов

❓ Какие есть фильтры поиска?
✅ Нажмите кнопку "Фильтр" для:
• Выбора типа контента (каналы, боты, приложения)
• Выбора категорий и подкатегорий
• Сортировки по популярности

❓ Как сбросить фильтры?
✅ Нажмите "Сбросить" в окне фильтров или переключитесь на другую вкладку и обратно.

⭐ СИСТЕМА РЕКОМЕНДАЦИЙ

❓ Как работают рекомендации?
✅ Система анализирует:
• Ваши клики по каналам
• Пройденный опрос интересов
• Похожих пользователей

❓ Почему нет рекомендаций?
✅ Пройдите опрос интересов и активно пользуйтесь поиском - система научится предлагать релевантный контент.
...
❓ Как открыть все каналы определенного типа?
✅ Нажмите на заголовок раздела ("Каналы", "Боты", "Приложения") на главной странице.

🔄 НАВИГАЦИЯ И ИНТЕРФЕЙС

❓ Как вернуться на главную?
✅ Используйте:
• Кнопку "Назад" в шапке
• Нижнюю панель навигации
• Сброс фильтров

❓ Для чего нужны вкладки внизу?
✅ • Поиск - главная страница с поиском
   • Рекомендации - персонализированные подборки
   • Настройки - управление приложением

❓ Как открыть канал?
✅ Нажмите на любую карточку канала - откроется новая вкладка с предпросмотром канала.

🔔 УВЕДОМЛЕНИЯ И НАСТРОЙКИ

❓ Как настроить уведомления?
✅ Перейдите в "Настройки" → "Уведомления" для настройки ежедневных напоминаний.

❓ В какое время приходят уведомления?
✅ Ежедневные уведомления о новых каналах приходят в 13:00.

❓ Можно ли сменить тему приложения?
✅ Тема автоматически соответствует настройкам Telegram.
...
✅ Напишите в поддержку через раздел "Настройки" → "Поддержка".

💡 СОВЕТЫ ПО ИСПОЛЬЗОВАНИЮ

• Регулярно проверяйте раздел рекомендаций
• Используйте фильтры для точного поиска
• Проходите опросы для улучшения рекомендаций
• Добавляйте понравившиеся каналы в избранное
        ...
        <div style="margin-bottom: 20px;">
            ...
            <div style="color: var(--tg-theme-text-color, #000000); max-height: 60vh; overflow-y: auto; padding-right: 8px;">
                <div style="margin-bottom: 16px;">
                    ...
                    <strong>❓ Что такое TC?</strong><br>
                    ✅ Это умное приложение для поиска каналов, ботов и мини-приложений в Telegram с персонализированными рекомендациями.<br><br>
                    
                    <strong>❓ Как работает приложение?</strong><br>
                    ✅ Приложение анализирует ваши предпочтения и предлагает релевантный контент через поиск и систему рекомендаций.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>РАБОТА С ПОИСКОМ</strong><br>
                    <strong>❓ Как искать контент?</strong><br>
                    ✅ Используйте строку поиска вверху экрана:<br>
                    • Введите ключевые слова и нажмите Enter<br>
                    • Используйте фильтры для уточнения результатов<br><br>
                    
                    <strong>❓ Какие есть фильтры поиска?</strong><br>
                    ✅ Нажмите кнопку "Фильтр" для:<br>
                    • Выбора типа контента (каналы, боты, приложения)<br>
                    • Выбора категорий и подкатегорий<br>
                    • Сортировки по популярности<br><br>
                    
                    <strong>❓ Как сбросить фильтры?</strong><br>
                    ✅ Нажмите "Сбросить" в окне фильтров или переключитесь на другую вкладку и обратно.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>СИСТЕМА РЕКОМЕНДАЦИЙ</strong><br>
                    <strong>❓ Как работают рекомендации?</strong><br>
                    ✅ Система анализирует:<br>
                    • Ваши клики по каналам<br>
                    • Пройденный опрос интересов<br>
                    • Популярные каналы<br><br>
                    
                    <strong>❓ Почему нет рекомендаций?</strong><br>
                    ✅ Пройдите опрос интересов и активно пользуйтесь поиском - система научится предлагать релевантный контент.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>📂 КАТЕГОРИИ И РАЗДЕЛЫ</strong><br>
                    <strong>❓ Какие категории доступны?</strong><br>
                    ...
                    <strong>❓ Как открыть все каналы определенного типа?</strong><br>
                    ✅ Нажмите на заголовок раздела ("Каналы", "Боты", "Приложения") на главной странице.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>НАВИГАЦИЯ И ИНТЕРФЕЙС</strong><br>
                    <strong>❓ Как вернуться на главную?</strong><br>
                    ✅ Используйте:<br>
                    • Нижнюю панель навигации<br>
                    • Сброс фильтров<br><br>
                    
                    <strong>❓ Для чего нужны вкладки внизу?</strong><br>
                    ✅ • Поиск - главная страница с поиском<br>
                    &nbsp;&nbsp;• Рекомендации - персонализированные подборки<br>
                    &nbsp;&nbsp;• Настройки - управление приложением<br><br>
                    
                    <strong>❓ Как открыть канал?</strong><br>
                    ✅ Нажмите на любую карточку канала - откроется новая вкладка с предпросмотром канала.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>УВЕДОМЛЕНИЯ И НАСТРОЙКИ</strong><br>
                    <strong>❓ Как настроить уведомления?</strong><br>
                    ✅ Перейдите в "Настройки" → "Уведомления" затем в бота для настройки ежедневных напоминаний.<br><br>
                    
                    <strong>❓ В какое время приходят уведомления?</strong><br>
                    ✅ Ежедневные уведомления о новых каналах приходят в 13:00.<br><br>
                    
                    <strong>❓ Можно ли сменить тему приложения?</strong><br>
                    ✅ Тема автоматически соответствует настройкам Telegram.
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>ПОДДЕРЖКА И ПРОБЛЕМЫ</strong><br>
                    <strong>❓ Приложение не загружается</strong><br>
                    ✅ Проверьте соединение с интернетом и перезапустите приложение.<br><br>
                    
                    <strong>❓ Не отображаются результаты поиска</strong><br>
                    ✅ Попробуйте:<br>
                    • Подождать немного<br>
                    • Очистить поисковый запрос<br>
                    • Сбросить фильтры<br>
                    • Проверить подключение к интернету<br><br>
                    
                    <strong>❓ Куда обращаться за помощью?</strong><br>
                    ✅ Напишите в поддержку через раздел "Настройки" → "Поддержка".
                </div>

                <div style="margin-bottom: 16px;">
                    <strong>💡 СОВЕТЫ ПО ИСПОЛЬЗОВАНИЮ</strong><br>
                    • Регулярно проверяйте раздел рекомендаций<br>
                    • Используйте фильтры для точного поиска<br>
                    • Проходите опросы для улучшения рекомендаций<br>
                </div>

                <div style="text-align: center; color: var(--tg-theme-hint-color, #666); font-style: italic;">
                    Наслаждайтесь использованием TCatalog! 🚀
                </div>
            </div>
        </div>
