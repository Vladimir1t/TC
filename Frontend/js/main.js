...
function handleSettingsClick(settingType) {
    ...
    switch (settingType) {
        ...
        case 'theme':
            ...
            const isDark = document.documentElement.classList.contains('tg-theme-dark');
            const themeName = isDark ? 'тёмная' : 'светлая';
            
            // Показываем сообщение о теме
            if (tg.showPopup) {
                tg.showPopup({
                    title: 'Тема приложения',
                    message: `Тема такая же, как и в настройках Telegram\nТекущая тема: ${themeName}`,
                    buttons: [{ type: 'ok' }]
                });
            } else {
...
Наслаждайтесь использованием TCatalog! 🚀`;
    ...
    if (tg.showPopup) {
        ...
        const formattedMessage = faqMessage
        ...
        modalContent.innerHTML = `
            <div style="margin-bottom: 15px;">
                <h3 style="margin: 0 0 15px 0; color: var(--tg-theme-text-color, black); text-align: center;">
                    ❓ Помощь по приложению
                </h3>
                <div style="color: var(--tg-theme-text-color, black);">
                    ${formattedMessage}
                </div>
            </div>
